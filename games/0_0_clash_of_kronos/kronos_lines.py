"""Lines evaluation for Clash of Kronos (dormant chest blocks lines like a mismatch)."""
...
class KronosLines(Lines):
    @staticmethod
    def _dormant_chest(sym: Symbol, chests_unlocked: bool) -> bool:
        return sym.defn.name == "CHEST" and not chests_unlocked

    @staticmethod
    def _chest_award_on_line(
        potential_line: list[Symbol],
        *,
        chests_unlocked: bool,
        paying_count: int,
        wild_only_line: bool,
    ) -> float:
        if not chests_unlocked or wild_only_line:
            return 0.0
        return sum(
            float(s.prize or 0) for s in potential_line[:paying_count] if s.defn.name == "CHEST"
        )
    @staticmethod
    def get_lines(
        ...
        config: Config,
        *,
        chests_unlocked: bool = False,
        wild_key: str = "wild",
        wild_sym: str = "WILD",
        multiplier_method: str = "symbol",
        global_multiplier: int = 1,
        tracker_stage: int = 1,
        tracker_multiplier: int = 1,
    ):
        ...
        for line_index in config.paylines.keys():
            ...
            if KronosLines._dormant_chest(first_sym, chests_unlocked):
                continue
            ...
            for reel in range(1, len(line)):
                ...
                if KronosLines._dormant_chest(sym, chests_unlocked):
                    break
            ...
            if base_win > 0 or wild_win > 0:
                if wild_win > base_win:
                    ...
                    chest_extra = KronosLines._chest_award_on_line(
                        potential_line,
                        chests_unlocked=chests_unlocked,
                        paying_count=wild_matches,
                        wild_only_line=True,
                    )
                    line_win, applied_mult = apply_mult(
                        ...
                        win_amount=wild_win + chest_extra,
                        positions=positions,
                    )
                    meta = {
                        ...
                        "lineMultiplier": max(1, int(applied_mult / max(global_multiplier, 1))),
                        "trackerStage": tracker_stage,
                        "trackerMultiplier": tracker_multiplier,
                    }
                    ...
                    chest_extra = KronosLines._chest_award_on_line(
                        potential_line,
                        chests_unlocked=chests_unlocked,
                        paying_count=matches + wild_matches,
                        wild_only_line=False,
                    )
                    line_win, applied_mult = apply_mult(
                        board,
                        multiplier_method,
                        global_multiplier=global_multiplier,
                        win_amount=base_win + chest_extra,
                        positions=positions,
                    )
                    meta = {
                        "lineIndex": line_index,
